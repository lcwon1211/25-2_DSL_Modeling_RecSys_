...
Original file is located at
    https://colab.research.google.com/drive/1l8oCDAMAShrutxXfd-usPF58INlGQ303
"""
...
BATCH_SIZE = 1024
LEARNING_RATE = 0.001
EMBEDDING_DIM = 32 # 임베딩 차원을 32로 설정
EPOCHS = 30
TOP_K_FOR_EVAL = [20, 100]
PATIENCE = 5
CHECKPOINT_PATH = 'best_model_warm_start.pth' # Warm Start 모델을 위한 체크포인트

# --------------------------------------------------------------------------
# 1단계: 데이터 로드 및 전처리 (PyG 형태로 변경)
# --------------------------------------------------------------------------
class KGDataPyG:
    """PyTorch Geometric(PyG)에 최적화된 형태로 데이터를 로드하고 전처리합니다."""
    def __init__(self, data_dir='.', train_file='train_interactions_warm.csv', val_file='validation_interactions_warm.csv'):
        ...
        try:
            ...
            raise FileNotFoundError(f"필수 파일({e.filename})을 찾을 수 없습니다. Warm Start 데이터 생성 스크립트를 먼저 실행하세요.")
...
# --------------------------------------------------------------------------
# 2단계: PyG 기반 KGAT 모델 정의
# --------------------------------------------------------------------------
class KGATLayerPyG(MessagePassing):
    """PyG의 MessagePassing을 상속받아 효율적으로 구현한 어텐션 레이어"""
    def __init__(self, in_dim, out_dim):
        super().__init__(aggr='add') # 이웃 메시지를 합산(add)
        self.W = nn.Linear(in_dim * 2, 1) # 어텐션 스코어는 스칼라 값이므로 out_dim=1
...
if __name__ == '__main__':
    ...
    try:
        # Warm Start용 데이터 파일을 불러옵니다.
        kg_data = KGDataPyG(train_file='train_interactions_warm.csv', val_file='validation_interactions_warm.csv')
    except FileNotFoundError as e:
        print(e); exit()

    print("\n2. Warm Start 모델 훈련 및 검증을 시작합니다...")
    ...
    print("\n" + "="*50); print("      최종 모델 성능 평가 결과 (Warm Start)"); print("="*50)
...
!pip uninstall torch torchvision torchaudio torch_geometric torch_scatter torch_sparse torch_cluster torch_spline_conv -y

# 1단계: 기본 분석 라이브러리 설치
!pip install pandas numpy tqdm scikit-learn matplotlib

# 2단계: PyTorch (GPU 버전) 및 PyTorch Geometric 코어 라이브러리 설치
# torch 2.1.2 버전을 기준으로 설치하여 호환성을 보장합니다.
!pip install torch==2.1.2 torch_geometric

# 3단계: PyTorch Geometric 보조 라이브러리 설치
# PyTorch 2.1.2와 CUDA 12.1에 완벽하게 호환되는 '완제품'을 직접 설치합니다.
...
TOP_K_FOR_EVAL = [20, 100]
PATIENCE = 5
CHECKPOINT_PATH = 'best_model_warm_start_v2.pth' # 개선된 모델을 위한 새 체크포인트
...
class KGATPyG(nn.Module):
    ...
    ***** 성능 개선 수정 사항 *****
    ...
    def get_final_embeddings(self, edge_index, edge_type):
        ...
        # 초기 임베딩과 결합 (Residual Connection)
...
if __name__ == '__main__':
    ...
    print("\n2. Warm Start 모델 훈련 및 검증을 시작합니다 (v2)...")
    train_dataloader = DataLoader(InteractionDataset(kg_data.train_df, kg_data.entity2id), batch_size=BATCH_SIZE, shuffle=True)
    model = KGATPyG(kg_data.n_entities, kg_data.n_relations, EMBEDDING_DIM).to(device)
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    criterion = nn.BCEWithLogitsLoss()
    ...
    best_model.load_state_dict(torch.load(CHECKPOINT_PATH))
    final_scores = evaluate_model(best_model, kg_data.validation_df, kg_data, device)
    print("\n" + "="*50); print("      최종 모델 성능 평가 결과 (Warm Start v2)"); print("="*50)
    for k in TOP_K_FOR_EVAL:
        print(f"Recall@{k} : {final_scores.get(f'Recall@{k}', 0):.4f}")
        print(f"NDCG@{k}   : {final_scores.get(f'NDCG@{k}', 0):.4f}")
        print(f"HitRate@{k}: {final_scores.get(f'HitRate@{k}', 0):.4f}")

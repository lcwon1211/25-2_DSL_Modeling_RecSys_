# -*- coding: utf-8 -*-
"""
Cold-Start DCN+DeepFM (v3) — fixed test users, wide/long interactions both supported
- Interactions:
  * Wide: columns = user_id, rank_1, rank_2, ... (each cell: item_id). Uses topk_pos ranks as positives.
  * Long: columns include user_id + (item_id|route_id). Optional rank/score/rating/timestamp used for sorting, then topk_pos/user.
- Test users: passed via --test_users "comma,separated", normalized to Int64->str for stable ID matching.
- Per-user positives: TopK (default 1000). Negatives sampled from items not in positives (train=neg_train, test=neg_test).
- Features: item_feats drop ['crossing_score_capped','crossing_score_normalized']; other numerics z-score inside encoder.
- Metrics (per-user, averaged): Hit@1, HR@10 (main), plus HR@K/NDCG@K/Recall@K (reference).
- Robustness: NaN/Inf defenses; ID normalization; safe model save (atomic); debug prints.
"""
import os, sys, math, random, argparse, shutil, re
import numpy as np
import pandas as pd
...
def get_args():
    ...
    ap.add_argument('--interactions', type=str, required=True)  # user_preferred_route.csv
    ap.add_argument('--test_users', type=str, required=True)    # "96,30,..."

    ap.add_argument('--topk_pos', type=int, default=1000)
    ap.add_argument('--neg_train', type=int, default=2000)
    ap.add_argument('--neg_test',  type=int, default=2000)
...
# -------------------- Cold Split --------------------
def _build_pos_from_wide(df: pd.DataFrame, topk_pos: int) -> pd.DataFrame:
    # Expect user_id + many rank_* columns
    rank_cols = [c for c in df.columns if re.match(r'^rank_\d+$', str(c))]
    if not rank_cols:
        # allow r1/top_1 variants
        rank_cols = [c for c in df.columns if re.match(r'^(top|r|rank)[_\-]?\d+$', str(c), flags=re.I)]
    if not rank_cols:
        raise ValueError("Wide interactions detected but no rank_* columns were found.")
    # sort columns by numeric suffix
    def keyf(c):
        m = re.search(r'(\d+)$', str(c)); return int(m.group(1)) if m else 10**9
    rank_cols = sorted(rank_cols, key=keyf)[:topk_pos]
    sub = df[['user_id'] + rank_cols].copy()
    long = sub.melt(id_vars=['user_id'], value_vars=rank_cols, var_name='rank_col', value_name='item_id')
    long.dropna(subset=['item_id'], inplace=True)
    long['item_id'] = long['item_id'].astype(str)
    long = long.drop_duplicates(subset=['user_id','item_id'])
    long['label'] = 1
    return long[['user_id','item_id','label']]

def cold_split_from_interactions(inter_df: pd.DataFrame, item_index: pd.Index, test_users_norm: set,
                                 topk_pos=1000, neg_train=2000, neg_test=2000, seed=42) -> Tuple[pd.DataFrame, pd.DataFrame]:
    # Detect format
    ucol = detect_id_col(inter_df, ["user_id","uid","USER_ID"])
    if ucol is None:  # fallback to first column as user_id
        ucol = inter_df.columns[0]
    is_wide = False
    if ucol in inter_df.columns:
        # wide if many columns starting rank_* or sheer width
        if any(str(c).startswith('rank_') for c in inter_df.columns) or len(inter_df.columns) > 100:
            is_wide = True

    if is_wide:
        inter_df = inter_df.rename(columns={ucol: "user_id"})
        inter_df['user_id'] = normalize_id(inter_df['user_id'])
        inter_df = inter_df[~inter_df['user_id'].isna() & (inter_df['user_id'] != '<NA>')]
        pos_long = _build_pos_from_wide(inter_df, topk_pos)
    else:
        icol = detect_id_col(inter_df, ["item_id","route_id","ROUTE_ID","rid","content_id"])
        if icol is None:
            raise ValueError("Could not detect item id column in interactions.")
        inter_df = inter_df.rename(columns={ucol: "user_id", icol: "item_id"})
        inter_df["user_id"] = normalize_id(inter_df["user_id"])
        inter_df["item_id"] = normalize_id(inter_df["item_id"])
        inter_df.dropna(subset=["user_id","item_id"], inplace=True)
        inter_df = inter_df[~inter_df["user_id"].eq("<NA>") & ~inter_df["item_id"].eq("<NA>")]
        # optional sort for topk
        sort_cols = []
        if "rank" in inter_df.columns:   sort_cols.append(("rank", True))
        if "score" in inter_df.columns:  sort_cols.append(("score", False))
        if "rating" in inter_df.columns: sort_cols.append(("rating", False))
        if "timestamp" in inter_df.columns: sort_cols.append(("timestamp", False))
        if sort_cols:
            by = [c for c,_ in sort_cols]; ascending = [asc for _,asc in sort_cols]
            inter_df = inter_df.sort_values(by=by, ascending=ascending, kind="mergesort")
        inter_df = inter_df.drop_duplicates(subset=["user_id","item_id"], keep="first")
        pos_long = inter_df.groupby("user_id", group_keys=False).head(topk_pos)[["user_id","item_id"]]
        pos_long["label"] = 1

    # Split users
    users_all = pos_long["user_id"].unique().tolist()
    test_users_in = [u for u in users_all if u in test_users_norm]
    if len(test_users_in) < len(test_users_norm):
        missing = sorted(list(test_users_norm - set(test_users_in)))
        if missing:
            print(f"[WARN] Test user(s) not found in interactions and will be ignored: {missing}")
    train_users = [u for u in users_all if u not in test_users_norm]

    # Build train/test pairs
    rng = np.random.default_rng(seed)
    item_arr = np.array(item_index)
    train_rows=[]; test_rows=[]
    pos_by_user = {u: set(g['item_id'].tolist()) for u,g in pos_long.groupby('user_id')}

    for u, target in [(u,'train') for u in train_users] + [(u,'test') for u in test_users_in]:
        pos_items = np.array(sorted(list(pos_by_user.get(u, set()))), dtype=item_arr.dtype)
        if pos_items.size == 0: continue
        pos_set = set(pos_items.tolist())
        mask = np.array([it not in pos_set for it in item_arr], dtype=bool)
        neg_pool = item_arr[mask]
        need = neg_train if target=='train' else neg_test
        if neg_pool.size > 0:
            n = min(need, neg_pool.size)
            neg_items = rng.choice(neg_pool, size=n, replace=False)
        else:
            neg_items = np.array([], dtype=item_arr.dtype)
        if target=='train':
            for it in pos_items: train_rows.append((u, it, 1))
            for it in neg_items: train_rows.append((u, it, 0))
        else:
            for it in pos_items: test_rows.append((u, it, 1))
            for it in neg_items: test_rows.append((u, it, 0))

    train_df = pd.DataFrame(train_rows, columns=["user_id","item_id","label"])
    test_df  = pd.DataFrame(test_rows,  columns=["user_id","item_id","label"])
...
def main():
    ...
    FINAL_MODEL_PATH = os.path.join(save_dir, f'coldstart_dcn_dfm_e{args.epochs}_k{args.eval_k}.pth')
    ...
    # detect id cols
    user_id_col = detect_id_col(user_raw, ["user_id","uid","USER_ID"]) or user_raw.columns[0]
    item_id_col_raw = detect_id_col(item_raw, ["item_id","route_id","ROUTE_ID"]) or item_raw.columns[0]
    ...
    # parse test users and normalize
    raw_ids = [s.strip() for s in args.test_users.split(',') if s.strip()!='']
    test_ids_series = pd.Series(raw_ids)
    test_users_norm = set(normalize_id(test_ids_series).tolist())

    # build pairs (support wide/long)
    train_pairs, test_pairs = cold_split_from_interactions(inter, item_raw.index, test_users_norm,
                                                           topk_pos=args.topk_pos,
                                                           neg_train=args.neg_train, neg_test=args.neg_test,
                                                           seed=args.seed)
    # Save generated pairs
    train_out = os.path.join(save_dir, "cold_train_pairs.csv")
    test_out  = os.path.join(save_dir, "cold_test_pairs.csv")
    train_pairs.to_csv(train_out, index=False)
    test_pairs.to_csv(test_out, index=False)
    print(f"[INFO] ColdStart 분할 생성 → {train_out} / {test_out}")
    print(f"[INFO] Train users: {train_pairs['user_id'].nunique()} | Test users: {test_pairs['user_id'].nunique()}")
    print(f"[INFO] Train pairs: {len(train_pairs)} (Pos={int((train_pairs['label']==1).sum())}, Neg={int((train_pairs['label']==0).sum())})")
    print(f"[INFO] Test  pairs: {len(test_pairs)}  (Pos={int((test_pairs['label']==1).sum())}, Neg={int((test_pairs['label']==0).sum())})")

    # encoder on TRAIN users only (cold-start)
    ...
    print("\n" + "="*70)
    print(f"  DCN+DeepFM 콜드스타트 훈련 시작 ({args.epochs} Epochs)")
    print("="*70)

    best_ndcg = 0.0
    for epoch in range(1, args.epochs+1):
        ld, lf = train_one_epoch(dcn, dfm, train_loader, opt_dcn, opt_dfm, crit, DEVICE, desc_text=f"Training Epoch {epoch}/{args.epochs}", debug_once=args.debug_fields_once)
        do_eval = (epoch % args.eval_every == 0) or (epoch == args.epochs)
        if do_eval:
            metrics = evaluate_userwise((dcn, dfm), test_pairs, user_raw, item_raw, enc, args, DEVICE)
            ...
            if (not args.no_save) and (metrics['NDCG@%d'%args.eval_k] > best_ndcg):
                ...
                try:
                    atomic_torch_save({'dcn': dcn, 'dfm': dfm}, os.path.join(save_dir, f'coldstart_dcn_dfm_e{args.epochs}_k{args.eval_k}.pth'))
                    print(f"[INFO] 최고 성능 모델 저장됨 (Best NDCG={best_ndcg:.4f})", flush=True)
    ...
    print("\n" + "="*70)
    print(f"훈련 완료! 최고 NDCG@{args.eval_k}: {best_ndcg:.4f}")
    print("="*70)
